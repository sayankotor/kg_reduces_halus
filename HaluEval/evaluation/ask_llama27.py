...
#start_emb = torch.load("/home/jovyan/shares/SR004.nfs2/chekalina/check_halu/ckpts/SOI2_llama2.pt", map_location=DEVICE)
#end_emb = torch.load("/home/jovyan/shares/SR004.nfs2/chekalina/check_halu/ckpts/EOI2_llama2.pt", map_location=DEVICE)
...
def ask_llama_with_prompt(question, prompt):
...
def ask_llama_with_kg_embeddings(question, prompt):
...
def ask_llama_with_kg(question, prompt, ent_embs):
...
def llama_dialog(prompt, response):
    print ("llama_dialog", flush = True)
    prompt_ids = tokenizer.encode(f"{prompt}", add_special_tokens=False, return_tensors="pt").to(device=DEVICE)
    prompt_embeddings = model.model.embed_tokens(prompt_ids).to(torch.bfloat16)
...
def llama_summary(prompt, summary):
...
def llama_summary_kg_embeddings(prompt, document, summary):
...
def dialogue_llama_with_knowledge(prompt, response, knowledge):
    prompt_ids = tokenizer.encode(f"{prompt}", add_special_tokens=False, return_tensors="pt").to(device=DEVICE)
    prompt_embeddings = model.model.embed_tokens(prompt_ids).to(torch.bfloat16)
    response_ids = tokenizer.encode(response, add_special_tokens=False, return_tensors="pt").to(device=DEVICE)
    response_embeddings = model.model.embed_tokens(response_ids).to(torch.bfloat16)
...
def llama_dialog_kg_embeddings(instruction, dialog, response):
